...
int main(int argc, char *argv[])
{
  ...
  cout << Element::TRIANGLE << " " << Element::QUADRILATERAL << endl;
  Mesh *mesh;
  ...
  /* 
  out.open("mesh.out");
  subdomains[4]->printmesh(out);
  out.close();

  int tbf = subdomains[4]->getnumbasis();
  cout << tbf << endl;
  for(int j=0; j<tbf; j++)
    {
      char fname[256];
      sprintf(fname, "basis_%d.out", j);
      out.open(fname);
      subdomains[4]->printbasis(out, j);
      out.close();
    }
  */
  ...
  //  Build MsRM
  //--------------------------------------------------------- 
  directMsRM msolver(subdomains, gbindex);
  ...
  cout << "H1error: " << fem->ComputeH1Error(exactderfunc, msol) << endl;
